    ...
    private fun initRecyclerViewArchive() {
        val items = ArrayList<AchievementDomain>().apply {
            add(
                AchievementDomain(
                    "Device Info",
                    listOf("It displays device information, including Root, CPU, Serial, System, HardDisk, and Kernel Info."),
                    "icon_achievement1"
                )
            )
            add(
                AchievementDomain(
                    "Hook Status",
                    listOf("It displays the hook status of the device."),
                    "icon_achievement2"
                )
            )
        }

        binding.viewArchive.layoutManager =
