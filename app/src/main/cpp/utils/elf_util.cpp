...
ElfW(Addr)
ElfImg::ElfLookup(std::string_view name,
                  uint32_t hash
) const {
...
ElfW(Addr)
ElfImg::GnuLookup(std::string_view name,
                  uint32_t hash
) const {
    ...
    if (symtabs_.empty()) {
        if (symtab_start != nullptr && symstr_offset_for_symtab != 0) {
            for (ElfW(Off) i = 0; i < symtab_count; i++) {
                ...
                const char *st_name = offsetOf<
                        const char *>(header, symstr_offset_for_symtab +
...
ElfW(Addr)
ElfImg::LinearLookup(std::string_view name) const {
    MayInitLinearMap();
...
ElfW(Addr)
ElfImg::PrefixLookupFirst(std::string_view prefix) const {
    MayInitLinearMap();
    if (auto i = symtabs_.lower_bound(prefix); i != symtabs_.end() &&
        ...
        LOGD("found prefix {%s} of {%s} {0x%x} in {%s} in symtab by linear lookup",
             std::string(prefix).c_str(), std::string(i->first).c_str(), i->second->st_value,
             elf.c_str());
        return i->second->st_value;
    } else {
        return 0;
...
ElfW(Addr)
ElfImg::getSymbOffset(std::string_view name,
                      uint32_t gnu_hash, uint32_t elf_hash) const {
    if (auto offset = GnuLookup(name, gnu_hash); offset > 0) {
        ...
        offset = LinearLookup(name); offset > 0) {
        LOGD("found {%s} {0x%x} in {%s} in symtab by linear lookup", std::string(name).c_str(),
             offset,
             elf.c_str());
        return offset;
    } else {
